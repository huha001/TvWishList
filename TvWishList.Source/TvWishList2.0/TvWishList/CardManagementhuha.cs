...
{
  public static class CardManagementhuha
  {   
    ...
    public static bool AddMapsToCards()
    {
        foreach (Card mycard in CardManagement.ListAllCards())
        {
            if (mycard.ChannelMaps.Count > 0)
            { 
                Log.Debug(mycard.ChannelMaps.Count.ToString()+" maps found for card "+mycard.Name);
                return true;
            }
        }

        foreach (Channel mychannel in ChannelManagement.ListAllChannels())
        {
            Log.Debug("**************" + mychannel.DisplayName + "********************");
            foreach (ChannelMap mymap in mychannel.ChannelMaps)
            {
                Log.Debug("----------------------------------------------------------");
                Log.Debug("mymap.Name = " + mymap.Channel.DisplayName.ToString());
                Log.Debug("mymap.IdChannel = " + mymap.IdChannel.ToString());
                Log.Debug("mymap.EpgOnly=" + mymap.EpgOnly.ToString());
                Log.Debug("mymap.Card.Name=" + mymap.Card.Name);
            }
        }
    ...
    public static bool CanViewTvChannel(Card card, int channelId)
    {
        ...
        {
            Log.Debug(mycard.ChannelMaps.Count.ToString() + " maps found for card " + mycard.Name);
            if (mycard.IdCard == card.IdCard)
            {
                Log.Debug("matching card id found");
                card = mycard;
                break;
            }
        }
        ...
        Log.Debug("anViewTvChannel: checking card=" + card.Name + " channelId=" + channelId.ToString());
        Log.Debug(card.ChannelMaps.Count.ToString() + " maps found for card " + card.Name);
        */
        //card = GetCard(card.IdCard);

        IList<ChannelMap> cardChannels = card.ChannelMaps;//bug are null and are not defined try to define for all cards and save

       bool ok =  cardChannels.Any(cmap => channelId == cmap.IdChannel && !cmap.EpgOnly);
