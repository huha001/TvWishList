...
//using GUIKeyboard = MediaPortal.GUI.Library.huha.GUIKeyboard;  //huha changed
namespace MediaPortal.GUI.Library //huha changed
{
    /// <summary>
    ...
    public abstract class GUIKeyboard : GUIControl
    {
        ...
        private string[] smsKeyMap =
      {
        ...
        public override void OnAction(Action action)
        {
            ...
            {
                Log.Debug("Closing in OnAction(Action action)");
                Close();
                return;
            }
            ...
            switch (action.wID)
            {
                ...
                case Action.ActionType.ACTION_MOVE_LEFT:
                    ...
                    {
                        Log.Debug("(Action.Move left) Backspace in OnAction(Action action)");
            ...
            switch (action.wID)
            {
                ...
                case Action.ActionType.ACTION_MOUSE_CLICK:
                    Log.Debug("Action Mouse Click in OnAction(Action action)");
                    ev = Event.EV_A_BUTTON;
                    UpdateState(ev);
                    break;

                case Action.ActionType.ACTION_SELECT_ITEM:
                    Log.Debug("Action Select item in OnAction(Action action)");
                    if (_currentKey == -1)
                    {
                        Close();
                        _pressedEnter = true;
                ...
                case Action.ActionType.ACTION_MOVE_LEFT:
                    Log.Debug("Action Move left in OnAction(Action action)");
                    ev = Event.EV_LEFT;
                    UpdateState(ev);
                    break;

                case Action.ActionType.ACTION_MOVE_RIGHT:
                    Log.Debug("Action Move right in OnAction(Action action)");
                    ev = Event.EV_RIGHT;
                    UpdateState(ev);
                    break;
                ...
                case Action.ActionType.ACTION_REMOVE_ITEM:
                    Log.Debug("Action Backspacet in OnAction(Action action)");
                    Press(Xkey.XK_BACKSPACE);
                    break;
                case Action.ActionType.ACTION_KEY_PRESSED:
                    if (action.m_key != null)
                    {
                        Log.Debug("Action key pressed in OnAction(Action action)");
                        ...
                        {
                            Log.Debug("Action Backspace key pressed in OnAction(Action action)");
                            Press(Xkey.XK_BACKSPACE);
                        }
                    }
                    break;
                case Action.ActionType.ACTION_TOGGLE_SMS_INPUT:
                    Log.Debug("Action toggle sms input in OnAction(Action action)");
            ...
            switch (_state)
            {
                case State.STATE_KEYBOARD:
                    switch (ev)
                    {
                        ...
                        // Navigation
                        case Event.EV_UP:
                            Log.Debug("Up in UpdateState");
                            MoveUp();
                            break;
                        case Event.EV_DOWN:
                            Log.Debug("Down in UpdateState");
                            MoveDown();
                            break;
                        case Event.EV_LEFT:
                            Log.Debug("Left in UpdateState");
                            MoveLeft();
                            break;
                        case Event.EV_RIGHT:
                            Log.Debug("Right in UpdateState");
                            MoveRight();
                            break;
                    }
                    break;
        ...
        //added huha
        public char ExtendCharacter
        {
            get { return _extendChar; }
            set { _extendChar = value; }
        }


        protected void LongTextProcessing(LongTextProcessTypes mycase)
        {
            Log.Debug("LongTextProcessing started\n");

            //Update projected characters for a full line
            float textboxwidth = GUIGraphicsContext.ScaleHorizontal((int)fTextBoxWidth);
            float fWidth = 0, fHeight = 0;

            //Cursorposition left or right
            bool cursorRightSide = false;
            if (_position > _textEntered.Length / 2)
            ...
            {
                #region CHAR_ADDED:
                case LongTextProcessTypes.CHAR_ADDED:
                    Log.Debug("Processing "+mycase.ToString()+":");

                    
                    _fontCharKey.GetTextExtent(_textEntered, ref fWidth, ref fHeight);
                    
                    Log.Debug("fWidth=" + fWidth.ToString());
                    Log.Debug("textboxwidth=" + textboxwidth.ToString());

                    if ((fWidth < textboxwidth)||(_textEntered.Length<3)) //nothing to do, too small or small enough
                    {
                        Log.Debug("nothing to do, too small or small enough");
                        return;
                    }

                    //reduce string
                    while ((fWidth >= textboxwidth) && (_textEntered.Length >= 3))  //string is too large - shorten by prefix and postfix
                    {

                        if (cursorRightSide == true) //cursor on right side
                        {
                            if (_textEntered[0] == _extendChar) //prefix exists already
                            {
                                _prefix += _textEntered[1].ToString();
                                _textEntered = _extendChar.ToString() + _textEntered.Substring(0, _textEntered.Length - 2);
                                if (_position > 0)
                                    _position--;
                            }
                            else //prefix does not exist
                            {
                                _textEntered = _extendChar.ToString() + _textEntered;
                                _position++;
                            }
                        }
                        else //cursor on left side
                        {
                            if (_textEntered[_textEntered.Length-1] == _extendChar) //postfix exists already
                            {
                                _postfix = _textEntered[_textEntered.Length-2].ToString() + _postfix;
                                _textEntered = _textEntered.Substring(0, _textEntered.Length - 3) + _extendChar.ToString();
                                //_position++;   no need to increase
                            }
                            else //prefix does not exist
                            {
                                _textEntered = _textEntered + _extendChar.ToString();
                            }
                        }
                        ...
                        Log.Debug("_textEntered =" + _textEntered);
                        Log.Debug("_position =" + _position);
                        Log.Debug("_postfix =" + _postfix);
                ...
                #region BACKSPACE:
                case LongTextProcessTypes.BACKSPACE:
                    Log.Debug("Processing " + mycase.ToString() + ":");
                    bool tooLong = false;
                    while (((_prefix != "") || (_postfix != "")) && (tooLong == false)) //remove pre and post fix characters until max length is reached again
                    {
                        if (cursorRightSide == true)  //cursor on right side
                        {
                            //remove prefix first
                            if (_prefix.Length > 0)
                            {
                                //check if longer string fits in box
                                _fontCharKey.GetTextExtent(_prefix[_prefix.Length-1].ToString() + _textEntered, ref fWidth, ref fHeight);
                                if (fWidth < textboxwidth)
                                {
                                    _textEntered = _prefix[_prefix.Length - 1].ToString() + _textEntered;//update string
                                    ...
                                    else
                                    {
                                        ...
                                        //remove extendchar at first position
                                        if ((_textEntered[0] == _extendChar) && (_textEntered.Length > 2))
                                        {
                                            _textEntered = _textEntered.Substring(1, _textEntered.Length - 2);
                                            if (_position > 0)
                                                _position--;
                                        }
                                    }
                            ...
                            //remove postfix second
                            if (_postfix.Length > 0)
                            {
                                //check if longer string fits in box
                                _fontCharKey.GetTextExtent(_textEntered + _postfix[0].ToString(), ref fWidth, ref fHeight);
                                if (fWidth < textboxwidth)
                                {
                                    _textEntered = _textEntered + _postfix[0].ToString();//update string

                                    if (_postfix.Length > 1)
                                        _postfix = _postfix.Substring(1, _postfix.Length - 1);
                                    else
                                    {
                                        _postfix = "";

                                        //remove extendchar at first position
                                        if ((_textEntered[_textEntered.Length-1] == _extendChar) && (_textEntered.Length > 2))
                                        {
                                            _textEntered = _textEntered.Substring(0, _textEntered.Length - 2);                                           
                                        }
                                    }
                                }
                                else
                ...
                #region CURSOR_RIGHT:
                case LongTextProcessTypes.CURSOR_RIGHT:
                    Log.Debug("Processing " + mycase.ToString() + ":");
                    if ((_prefix == "") && (_postfix == "")) //nothing to do
                    {
                        Log.Debug("No prefix and no postfix - returning");
                        return;
                    }

                    if (_position == _textEntered.Length)
                    {
                        Log.Debug("Cursor at last position - returning");
                        return;
                    }

                    //remove right char
                    char tempCharR = _textEntered[_position];

                    Log.Debug("Removing Char=" + tempCharR.ToString());
                    _textEntered = _textEntered.Substring(0, _position);
                    if (_textEntered.Length > _position + 1)
                    {
                        _textEntered +=_textEntered.Substring(_position + 1, _textEntered.Length - 1 - _position);
                    }
                    LongTextProcessing(LongTextProcessTypes.BACKSPACE);


                    //insert right char again
                    Log.Debug("Adding Char=" + tempCharR.ToString());
                    _textEntered = _textEntered.Substring(0, _position) + tempCharR.ToString()+_textEntered.Substring(_position, _textEntered.Length - 1 - _position);

                    LongTextProcessing(LongTextProcessTypes.CHAR_ADDED);
                ...
                #region CURSOR_LEFT:
                case LongTextProcessTypes.CURSOR_LEFT:
                    Log.Debug("Processing " + mycase.ToString() + ":");
                    if ((_prefix == "") && (_postfix == "")) //nothing to do
                        return;

                    if (_position == _textEntered.Length - 1)
                    {
                        Log.Debug("Cursor at last position - returning");
                        return;
                    }
                ...
                #region DONE:
                case LongTextProcessTypes.DONE:
                    Log.Debug("Processing " + mycase.ToString() + ":"); //must be called before returning string at the end

                    if ((_prefix != "")&&(_textEntered.Length > 0))
                    {
                        if (_textEntered[0] != _extendChar)
                        {
                            Log.Error("Error: Extendcharacter not found in text string at the beginning");
                            _position = _position + _prefix.Length;
                        }
                        else
                        {
                            _textEntered = _textEntered.Substring(1, _textEntered.Length - 1); //remove extend character at the beginnong
                            _position = _position + _prefix.Length - 1;

                        }
                    }

                    if ((_postfix != "") && (_textEntered.Length > 0))
                    {
                        if (_textEntered[_textEntered.Length-1] != _extendChar)
                        {
                            Log.Error("Error: Extendcharacter not found in text string");
                        }
                        else
                        {
                            _textEntered = _textEntered.Substring(0, _textEntered.Length - 1); //remove extend character at the end
                        }
                    }

                    _textEntered = _prefix + _textEntered + _postfix;
                    Log.Debug("_textEntered =" + _textEntered);
                    Log.Debug("_position =" + _position);
                    Log.Debug("_postfix =" + _postfix);
                    Log.Debug("_prefix =" + _prefix);
                ...
                #region NOTHING:
                case LongTextProcessTypes.NOTHING:
                    Log.Debug("Processing " + mycase.ToString() + ":");              
                    break;
                    #endregion
            }
            Log.Debug("LongTextProcessing completed\n");
        }

        public enum LongTextProcessTypes
        {
            CHAR_ADDED = 0,
            BACKSPACE,
            CURSOR_RIGHT,
